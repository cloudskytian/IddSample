...
int s_SampleDefaultModes[3] = { 1920,1080,60 };

#include <tchar.h>
void LoadSampleDefaultModes()
{
    HKEY hKey;
    ...
    if (ERROR_SUCCESS == ::RegOpenKeyEx(HKEY_CURRENT_USER, _T("SOFTWARE\\IddSampleDriver"), 0, KEY_READ, &hKey))
    {
        ...
        {
            s_SampleDefaultModes[0] = dwValue;
        }
        if (RegQueryValueEx(hKey, _T("Height"), NULL, &dwType, (LPBYTE)&dwValue, &dwSize) == ERROR_SUCCESS && dwType == REG_DWORD)
        {
            s_SampleDefaultModes[1] = dwValue;
        }
        if (RegQueryValueEx(hKey, _T("VSync"), NULL, &dwType, (LPBYTE)&dwValue, &dwSize) == ERROR_SUCCESS && dwType == REG_DWORD)
        {
            s_SampleDefaultModes[2] = dwValue;
        }
        RegCloseKey(hKey);
    }
};
...
NTSTATUS IddSampleMonitorGetDefaultModes(IDDCX_MONITOR MonitorObject, const IDARG_IN_GETDEFAULTDESCRIPTIONMODES* pInArgs, IDARG_OUT_GETDEFAULTDESCRIPTIONMODES* pOutArgs)
{
    ...
    else
    {
        pInArgs->pDefaultMonitorModes[0] = CreateIddCxMonitorMode(
            s_SampleDefaultModes[0],
            s_SampleDefaultModes[1],
            s_SampleDefaultModes[2],
...
NTSTATUS IddSampleMonitorQueryModes(IDDCX_MONITOR MonitorObject, const IDARG_IN_QUERYTARGETMODES* pInArgs, IDARG_OUT_QUERYTARGETMODES* pOutArgs)
{
    ...
    TargetModes.push_back(CreateIddCxTargetMode(s_SampleDefaultModes[0], s_SampleDefaultModes[1], s_SampleDefaultModes[2]));
